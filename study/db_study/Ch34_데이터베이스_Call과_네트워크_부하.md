...
음… 뭔가 여태 DB에 대해 배운 내용이 많았는데, 솔직히 처음에는 모르는 단어도 많았고 잘 이해가 안 됐던 것에 비해 뒤로 오면 올수록 뭔가 퍼즐이 맞춰지는 기분이 들면서 이해도가 높아진 것 같다.

뭔가 진행되면서 어플리케이션과 DB 간의 통신, 그리고 DB가 사용자로부터 SQL을 받고 어떻게 처리해서 반환하는지까지의 과정을 점점 알게되다보니,

그동안 “올바른 SQL 작성은 이렇다”, “대규모 데이터 처리를 위해서는 이렇게 해야된다“ 라는 식의 설명들이 어디서 기인했고 왜 그런 식으로 말했는지에 대해 전보다 훨씬 감이 잡히는 것 같다.

내공이 점점 늘어가는 기분이 든다… 솔직히 DB를 정말 기초만 알고 있었을 때는 SQL로 CRUD만 할 줄 알면 되지 뭐 그렇게 깊게 배워야 하나? 라는 생각이 들었는데 막상 배워보니 DB를 안 배운 사람들이 아주 귀여워 보이고 스터디를 시작하길 잘한 것 같다는 생각이 든다 ㅋㅋ 계속해서 열심히 해야겠다.